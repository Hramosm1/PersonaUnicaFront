import { Listado, Perfil } from 'src/app/content/perfiles-creados/tablas-model';
...
import { queryPerfiles } from '@shared/models/Perfiles';
...
export class ApiService {
	...
	public mantenimientos(
		...
		console.log(body);
		return this.http[tipo](
			`${environment.serverUrl}${mantenimiento}/${id}`,
			body,
	...
	public mantenimientosDelete(
		...
		return this.http.delete(`${environment.serverUrl}${mantenimiento}/${id}`, {
			headers: this.headers,
		});
	}
	//=================================================================================
	public getTipos<T>(
		...
		return this.http
			.get<Array<T>>(`${environment.serverUrl}tipos/${tipo}/`, {
				headers: this.headers,
			})
			.pipe(timeout(10000));
	}
	public GetPerfil(id: string): Observable<Perfil> {
		...
		return this.http
			.get<Perfil>(environment.serverUrl + 'perfiles/' + id, {
				headers: this.headers,
			})
			.pipe(timeout(18000));
	}
	public GetPerfiles(page: Page, query?: queryPerfiles): Observable<Listado> {
		const params = query
			? new HttpParams({ fromObject: query as any })
			: undefined;
		return this.http.get<Listado>(
			`${environment.serverUrl}perfiles/${page.take}/${page.pageNumber} `,
			{
				headers: this.headers,
				params,
			}
	...
	public updatePerfil(
		id: string,
		body: any
	): Observable<{ error: boolean; mensaje?: string }> {
		console.log(body);
		return this.http.put<{ error: boolean; mensaje?: string }>(
			environment.serverUrl + 'perfiles/' + id,
			body,
			{
				headers: this.headers,
			}
	...
	public PostRespuestaPersonaUnica(body: any) {
		return this.http
			.post(environment.serverUrl + 'perfiles', body, { headers: this.headers })
			.pipe(timeout(20000));
	}
