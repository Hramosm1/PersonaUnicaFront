...
import { Empresas, TiposOrigen } from '@app/content/mantenimiento/inputs/models';
...
export class FormEmpleoComponent implements OnInit {
  ...
  openModal(template: TemplateRef<any>) {
    this.modalRef2 = this.modalService.show(template, { class: 'modal-xl modal-dialog-scrollable' });
  }
  ...
  submit() {
    if (this.titulo == 'Editar') {
      ...
      this.api.mantenimientos('put', 'empleos', this.empleo.value, this.idEditar).subscribe((re: any) => {
        if (re.error) {
        } else {
          Swal.fire({
            position: 'top-end',
      ...
      this.rout.params.subscribe((re) => {
        console.log('creando');
        this.api
          .mantenimientos('post', 'empleos', { idPerfil: re.id, ...this.empleo.value }, '')
          .subscribe((re: any) => {
            if (re.error) {
            } else {
              Swal.fire({
